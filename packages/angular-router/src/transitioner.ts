    ...
    tap((isLoading) => {
      ...
      if (previousIsLoading && !isLoading) {
        this.router.emit({
          type: 'onLoad',
          ...getLocationChangeInfo(
            this.router.stores.location.state,
            this.router.stores.resolvedLocation.state,
          ),
        })
        this.router.stores.status.setState(() => 'idle')
      }
    }),
  )
  ...
  private pending$ = this.isAnyPending$.pipe(
    tap((isAnyPending) => {
      ...
      if (previousIsAnyPending && !isAnyPending) {
        ...
        batch(() => {
          this.router.stores.status.setState(() => 'idle')
          this.router.stores.resolvedLocation.setState(
            () => this.router.stores.location.state,
          )
        })

        if (changeInfo.hrefChanged) {
          handleHashScroll(this.router)
        }
      }
    }),
  )
